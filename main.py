...
with col1:
    st.subheader("Categorize with Sales")
...
linechart = pd.DataFrame(
    filtered_df.groupby(filtered_df["month_year"].dt.strftime("%b-%Y"))["Sales"].sum()).reset_index()
...
st.subheader(":point_right: Monthly Sub_Category Sales Summary")
...
data1 = px.scatter(data_frame=filtered_df, x="Sales", y="Profit", size="Quantity", color=filtered_df.Category)
